...
def calculate_remaining_lifespan(manufacturing_date_str, avg_lifespan):
    try:
        # Automatically detect and parse the date in any format
        manufacturing_date = parser.parse(manufacturing_date_str)
    except Exception as e:
        print(f"Error: Unrecognized date format ({manufacturing_date_str}) - {e}")
        return avg_lifespan  # Return avg lifespan as fallback

    # Get the current date
    current_date = datetime.now()

    # Calculate vehicle age in years
    age_years = (current_date - manufacturing_date).days / 365.25

    # Compute remaining lifespan
    remaining_lifespan = avg_lifespan - age_years

    return max(0, remaining_lifespan)  # Ensure lifespan is never negative
...
def manual_data_fetch_and_regression(extracted_details, condition_value):
    try:
        ...
        X = filtered_df[['Avg Lifespan (years)', 'Demand']]
        ...
        input_features = pd.DataFrame([[remaining_lifespan, demand]], columns=['Avg Lifespan (years)', 'Demand'])
...
# Upload bill and manual form handling
...
def manual_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Save files to storage and get their paths
            product_image_path = None
            product_video_path = None
